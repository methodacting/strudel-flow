...
import { deserializeState } from '@/lib/state-serialization';
...
export function PresetPopover() {
  ...
  const handlePresetLoad = (preset: (typeof presets)[0]) => {
    ...
    const state = deserializeState(preset.state);
    if (!state) {
      setLoadedPreset('Failed to load preset');
      setTimeout(() => setLoadedPreset(null), 2000);
      return;
    }
    ...
    setLoadedPreset(`${preset.name} - Loaded`);
    setTimeout(() => setLoadedPreset(null), 2000);
          ...
          <div className="space-y-2">
            {presets.map((preset, index) => (
              ...
              >
                ...
                <Button
                  size="sm"
                  variant={preset.state ? 'default' : 'ghost'}
                  className="ml-2 h-8"
                  onClick={() => handlePresetLoad(preset)}
                  disabled={!preset.state}
                >
                  {preset.state ? 'Load' : 'Soon'}
                </Button>
              </div>
            ))}
          </div>
