...
ADSRNode.strudelOutput = (node: AppNode, strudelString: string) => {
	...
	if (attack !== 0.1)
		result = result ? `${result}.attack("${attack}")` : `attack("${attack}")`;
	if (decay !== 0.1)
		result = result ? `${result}.decay("${decay}")` : `decay("${decay}")`;
	if (sustain !== 0.7)
		result = result ? `${result}.sustain("${sustain}")` : `sustain("${sustain}")`;
	if (release !== 0.2)
		result = result ? `${result}.release("${release}")` : `release("${release}")`;
