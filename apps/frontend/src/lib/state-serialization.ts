...
export function deserializeState(compressed: string): SerializableState | null {
  try {
    const tryDecompress = (value: string) => decompressFromBase64(value);
    let jsonString = tryDecompress(compressed);
    if (!jsonString && compressed.includes("%")) {
      jsonString = tryDecompress(decodeURIComponent(compressed));
    }
